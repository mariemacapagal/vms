          ...
          <div class="row">
            <div class="col-md-3 col-lg-2 mb-3 mb-md-0">
              ...
              <select name="table_selection" id="table_selection" class="form-select" required>
                <option value="visitors" {{ request('table_selection') === 'visitors' ? 'selected' : '' }}>Visitors</option>
                <option value="visit_logs" {{ request('table_selection') === 'visit_logs' ? 'selected' : '' }}>Visit Logs</option>
              </select>
            </div>
            ...
            @else
            <div class="col-auto mt-4 pt-1">
              <a href="{{ route('exportToCsv', ['table_selection' => $tableSelection, 'start_date' => $start_date, 'end_date' => $end_date_full]) }}" class="btn btn-primary">Save CSV</a>
            </div>
            @endif
          </div>
        </form>
    ...
    <table class="table table-striped">
      ...
      <tbody class="table-border-bottom-0">
        ...
        <tr>
          ...
          <td>{{ $data->log_date }}</td>
          <td><span class="badge me-1 {{ $data->status == 'OUT' ? 'bg-label-success' : 'bg-label-info'}}">{{ $data->status }}</span></td>
          @endif
        </tr>
    @endforeach
      </tbody>
